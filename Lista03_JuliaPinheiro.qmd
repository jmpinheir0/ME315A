...
Ao analisarmos as tabelas, podemos observar que todas apresentam as mesmas variáveis, `DNO` e `SALARY`. Porém, como a Tabela 1 e a Tabela 2 possuem exatamente as mesmas 6 linhas, o `DISTINCT` não removeu nenhuma linha. A única diferença ocorre na Tabela 3, que aplica uma ordenação crescente por `DNO` e `SALARY`.
...
Sim, são equivalentes, pois ambas filtram primeiro a condição de `STATE = 'MA'` e depois condicionam se querem `ACRES > 1000` ou `FEE = 0.0`
...
Sim, são equivalentes, pois ambas condionam se querem `STATE = 'MA'` ou `ACRES > 1000` e `FEE = 0.0`
...
Sim, a 1ª e a 3ª são equivalentes. Elas exibem todas as reservas, exceto aquelas que possuem simultaneamente menos de 50 acres e são de 'MA'. A 2ª é a diferente, pois ela exige que a reserva não seja de 'MA' e não tenha menos de 50 acres ao mesmo tempo.
...
Sim, a 1ª e a 3ª são equivalentes. Ambas excluem qualquer reserva que seja de 'MA' ou que tenha menos de 50 acres. A 2ª é a diferente por ter mantido o operador OR após as negações.
